...
class SentimentConsumer(BaseConsumer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"NER keyword map built with {len(self.keyword_to_asset_map)} keywords.")
        ...
        try:
            for message in self.consumer:
                try:
                    ...
                    # Preprocess text
                    text_to_analyze = [
                        self.preprocessor.preprocess(data.text),
                        self.preprocessor.preprocess(data.content),
                    ]
                    text_to_analyze = [t for t in text_to_analyze if t] # Get rid of None or empty
                    if not text_to_analyze:
                        logger.warning("Skipping message, no text found. Asset: %s", data.asset_name)
                        continue
                    ...
                    self.producer.send(self.out_topic, value=output_message.model_dump())
